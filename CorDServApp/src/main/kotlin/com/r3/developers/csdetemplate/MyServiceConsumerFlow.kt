...
import com.r3.developers.csdetemplate.service.use.TestServiceConsumer
...
class MyServConsFlow: RPCStartableFlow {
    ...
    override fun call(requestBody: RPCRequestData): String {
        ...
        log.info("MSCF: MyServConsFlow.call() called")

        // Show the requestBody in the logs - this can be used to help establish the format for starting a flow on corda
        log.info("MSCF: requestBody: ${requestBody.getRequestBody()}")

        // Deserialize the Json requestBody into the MyServConsFlowStartArgs class using the JsonSerialisation Service
        val flowArgs = requestBody.getRequestBodyAs(jsonMarshallingService, MyServConsFlowStartArgs::class.java)
        ...
        val servConsMessage = ServConsMessage(otherMember, "Hello from $ourIdentity.")
        ...
        val test = TestServiceConsumer.Companion.testMe(servConsMessage.message)
        log.info(test)
        log.info("<<<")

        // Start a flow session with the otherMember using the FlowMessaging service
        // The otherMember's Virtual Node will run the corresponding MyServConsFlowResponder responder flow
        val session = flowMessaging.initiateFlow(otherMember)

        // Send the Payload using the send method on the session to the MyServConsFlowResponder Responder flow
        session.send(servConsMessage)

        // Receive a response from the Responder flow
        val response = session.receive(ServConsMessage::class.java)
...
class MyServConsFlowResponder: ResponderFlow {
    ...
    override fun call(session: FlowSession) {
        ...
        // Useful logging to follow what's happening in the console or logs
        log.info("MSCF: MyServConsResponderFlow.call() called")

        // Receive the payload and deserialize it into a Message class
        val receivedServConsMessage = session.receive(ServConsMessage::class.java)

        // Log the message as a proxy for performing some useful operation on it.
        log.info("MSCF: Message received from ${receivedServConsMessage.sender}: ${receivedServConsMessage.message} ")
...
{
    "clientRequestId": "rsc_1",
    "flowClassName": "com.r3.developers.csdetemplate.MyServConsFlow",
    "requestData": {
